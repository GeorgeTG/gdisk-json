...
void GPTData::DisplayGPTData(int toJson) {
    ...
    if (toJson) {
        Json::Value root, tempPart;
        root["device"] =  device;
        root["sectors"] =  diskSize;
        root["size"] = BytesToIeee(diskSize, blockSize);
        root["sector_size"] = blockSize;
        root["guid"] = mainHeader.diskGUID.AsString();
        root["max_parts"] = numParts;
        root["first_sector"] = mainHeader.firstUsableLBA;
        root["last_sector"] = mainHeader.lastUsableLBA;
        root["alignment"] = sectorAlignment;
        totalFree = FindFreeBlocks(&i, &temp);
        root["free_space"] = BytesToIeee(totalFree, blockSize);
