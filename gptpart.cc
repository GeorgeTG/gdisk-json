...
break_converter : ;
    ...
    Json::Value GPTPart::GetJson(int partNum, uint32_t blockSize){
        ...
        if (firstLBA != 0) {
            ...
            partJson["number"] = partNum;
            partJson["start"] = firstLBA;
            partJson["end"] = lastLBA;
            partJson["size"] = sizeInIeee;
            partJson["type"] = partitionType.GetHexType();
#ifdef USE_UTF16
            GetDescription().extractBetween(0, 23, description);
            partJson["name"] = description;
#else
            ...
            partJson["name"] = GetDescription().substr( 0 , i ) ;
#endif
            return partJson;
        } // if
